...
- 실행 시각: **평일 08:30 KST**(주 틱) + **09:30 KST**(백업 틱). cron은 UTC라 `'30 23 * * 0-4'` / `'30 0 * * 1-5'` 두 줄. 백업은 GitHub 스케줄 스킵 대비 — 멱등성 덕에 중복 발송 없음.
...
**09:00 KST 이후로 옮기면 UTC가 당일로 넘어가므로 요일도 `1-5`로 바꿔야 한다.** 놓치면 월요일이 빠지고 토요일에 알림이 간다. 백업 틱 `'30 0 * * 1-5'`(09:30 KST)가 요일 범위만 다른 이유가 바로 이것 — 09:30 KST는 UTC 당일 00:30이라 날짜가 안 밀린다. 두 틱 모두 KST 월~금을 가리킨다.

**백업 틱은 GitHub 스케줄의 지연·스킵 대비다.** 하루 크롤을 통째로 놓치면 그날의 날짜 기반 알림이 영구 유실되므로, 시각을 벌려 두 번 친다. 멱등성 키가 사건일 기준이라 둘 다 떠도 중복 발송은 없다(두 번째는 대개 no-op). 시각을 바꿀 땐 두 줄 모두, 각자의 요일 매핑을 재계산할 것.
