...
> 이 문서는 **새 세션의 AI가 이것만 읽고 바로 작업할 수 있도록** 쓰였다.
> 코드를 고쳤으면 이 문서도 같이 고친다. 특히 **불변식**과 **함정** 절.
...
테스트 110개: `tests/{parse,rules,notify,pipeline,brokers,detail}.test.ts`.
...
| `D_DAY` | 청약 시작일 == 오늘 |
| `LAST_DAY` | 청약 마감일 == 오늘 |
| `SCHEDULE_CHANGED` | 이전 스냅샷 대비 일정 변경 |
...
**알려진 한계:** 하루치 실행이 통째로 실패하면 그날의 `LAST_DAY` 알림은 복구되지 않는다. 같은 날 `workflow_dispatch` 수동 실행이 유일한 복구 경로다.
