...
- 실행 시각: **평일 08:30 KST**(주 틱, `'30 23 * * 0-4'`) + **매일 07:30 KST**(실험 틱, `'30 22 * * *'`). cron은 UTC. 실험 틱은 GitHub 스케줄의 큰 지연(주 틱 실측 +1h25m)을 예정 앞당김으로 보정할 수 있는지 관측 중 — 멱등성 덕에 중복 발송 없음.
...
| 트리거 | 조건 | 메시지 순서 |
|---|---|---|
| `LISTING_DAY` | **상장일 == 오늘** | 1 (맨 위) |
| `LAST_DAY` | 청약 마감일 == 오늘 | 2 |
| `D_DAY` | 청약 시작일 == 오늘 | 3 |
| `D_MINUS_1` | 청약 시작일 == 내일 | 4 |
| `SCHEDULE_CHANGED` | 이전 스냅샷 대비 일정 변경 | 5 |

**`LISTING_DAY`가 맨 위인 이유:** 상장일 = 매도 가능일이다. 청약은 놓쳐도 다음 기회가 있지만 상장일을 놓치면 실제 손익이 갈린다. 08:30 발송이라 장 시작(09:00) 30분 전에 도착한다.
...
`cron`은 **항상 UTC**다. 주 틱 `'30 23 * * 0-4'`에서 요일이 하루 당겨진 것은 **의도된 것** — 23:30 UTC는 아직 전날이라 UTC 일~목 = KST 월~금이다.

**09:00 KST 이후로 옮기면 UTC가 당일로 넘어가므로 요일도 `1-5`로 바꿔야 한다.** 놓치면 월요일이 빠지고 토요일에 알림이 간다. 반대로 **09:00 KST 이전(UTC 자정 이전)이면 UTC/KST 요일이 같다** — 그래서 07:30 KST 실험 틱은 `'30 22 * * *'`(22:30 UTC)로 요일 매핑 걱정이 없다.

**실험 틱 `'30 22 * * *'`(07:30 KST, 매일)의 목적:** GitHub 스케줄 지연이 날마다 일정하다는 관측(주 틱 +1h25m, [[gh-actions-cron-delay-watch]])에 기대어, 예정을 앞당기면 실제 도착이 08:30 근처로 당겨지는지 본다. 매일 도는 것은 주말 포함 데이터를 빨리 모으기 위함이고, 멱등성 키가 사건일 기준이라 중복 발송은 없다. **실측 도착 시각을 며칠 모은 뒤**, 08:30을 정확히 맞추도록 예정 시각을 조정하거나(정시성이 확보되면 주 틱을 대체), 안 되면 외부 트리거로 전환한다. 시각을 바꿀 땐 두 줄 모두 각자의 요일 매핑을 재계산할 것.
