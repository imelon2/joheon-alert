...
### 원하는 날짜 알림을 지금 텔레그램으로 받아보기

```bash
pnpm send:preview --date=2026-07-30   # 그 날짜 기준 알림을 실제로 발송
pnpm send:preview                     # 오늘(KST) 기준
```

**state를 전혀 쓰지 않습니다.** 그래서 같은 날짜로 몇 번이든 다시 보낼 수 있고, 정작 그날이 왔을 때 진짜 알림도 정상 발송됩니다. 지정 날짜로 보내면 본문 맨 위에 `🧪 미리보기` 가 붙어 진짜 알림과 구분됩니다.

`main.ts` 의 `--date` 는 여전히 `--dry-run` 전용입니다. 가짜 날짜로 실제 발송하면 `{no}:{type}:{eventDate}` 키가 `notified.json` 에 박혀 **그날의 진짜 알림이 영구히 억제되기** 때문입니다. 이 스크립트는 파이프라인을 dry-run으로 돌려 state 쓰기를 막고, 렌더링된 메시지만 따로 보냅니다.
